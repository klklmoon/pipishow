<?php
$this->breadcrumbs = array('运营管理','赠品发放记录');
$types = $consumeSer->getGiveawayType();
$sources = $_commonSer->getBagSource();
?>
			...
			<table class="table table-bordered" id="gift_list_table">
				...
				<tbody>
				  	...
				  	<tr>
						...
						<td><?php echo $uinfo['gift_id']['zh_name']?></td>
						<td><?php echo $uinfo['num']?></td>
						<td><?php echo $uinfo['info']['remark'];?></td>
						<td><?php echo empty($sources[$uinfo['source']])?'':$sources[$uinfo['source']];?></td>
					</tr>
				  	<?php 
				  			}
				  		}
