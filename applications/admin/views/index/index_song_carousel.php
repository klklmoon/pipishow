<?php
$this->breadcrumbs = array('首页管理','唱区轮播');
?>
...
<div class="row-fluid sortable ui-sortable">
	<div class="box span12">
		...
		<div class="box-header well" data-original-title="">
			<h2><i class="icon-edit"></i>唱区轮播</h2>
		</div>
		<?php }?>
		<div class="box-content">
			...
			<?php }?>
			<form class="form-horizontal" method="post" action="<?php echo $this->createUrl('index/songcarousel',array('op'=>'addSongCarousel'));?>" enctype="multipart/form-data">
				<fieldset>
				  <div class="control-group" id="show_addEffect">
				      <label class="control-label"></label>
				      <div class="controls">
						  <a href="javascript:;" id="addEffect" class="btn">+唱区轮播</a>
				      </div>
				   </div>
				  
				  <dl style="border-top:1px #999999 dotted"></dl>
				  	唱区轮播推荐列表操作（最大上限为5张图片）
					...
					<dl class="effect">
						...
						<dt style="width:230px;">
							<em title="查看明星图片" class="icon icon-color icon-link" imgSrc="<?php echo $this->operateSer->getOperateImgUrl($data['piclink']);?>"></em>
...
$(document).ready(function(){
	...
	$("dt .icon-remove").click(function(){
		...
		if(operateId){
			$.ajax({
				type:'post',
				url:'<?php echo $this->createUrl('index/songcarousel');?>',
				dataType:'html',
