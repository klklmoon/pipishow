<?php
$this->breadcrumbs = array('首页管理','首页厨窗');
?>
<style type="text/css">
...
<div class="row-fluid sortable ui-sortable">
	<div class="box span12">
		...
		<div class="box-header well" data-original-title="">
			<h2><i class="icon-edit"></i>添加厨窗</h2>
		</div>
		<?php }?>
		<div class="box-content">
			...
			<?php }?>
			<form class="form-horizontal" method="post" action="<?php echo $this->createUrl('index/homewindow',array('op'=>'addHomeWindow'));?>" enctype="multipart/form-data">
				<fieldset>
				 <!--  
				 ...
				 -->
				  ...
				  <div class="control-group" id="show_addEffect">
				      ...
				      <div class="controls">
						  <a href="javascript:;" id="addEffect" class="btn">+添加厨窗</a>
				      </div>
				   </div>
				  ...
				  <dl style="border-top:1px #999999 dotted"></dl>
					...
					<dl class="effect"> </dl>
					<?php if(isset($homeData)){?>
					<?php foreach($homeData as $k=>$data){?>
...
$(document).ready(function(){
		...
		if(doteyName){
			...
			$.ajax({
				...
				success:function(msg){
					...
					if(data[0] == 1){
						$("#valid_dotey_info_noty").html(data[2]+'&nbsp;&nbsp; 验证通过').show();
	...
	$("#addEffect").click(function(){
		...
		//if(doteyId && doteyName){
			if(effectNum>5){
				alert('最多只能添加5个厨窗推荐');
			}else{
				if($('#'+doteyId).attr('class') == 'effect' ){
					$("#valid_dotey_info_noty").html(doteyName+' 已经存在于厨窗列表中，不能重复添加').show();
					...
					//effectHtml += "<input name='indexForm[target_name][]' type='text' readonly='true' id='indexForm[target_name]["+effectNum+"]' class='input-small focused' size='10' value='"+doteyName+"'/></dd>";
