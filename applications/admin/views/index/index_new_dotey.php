<?php
$this->breadcrumbs = array('首页管理','最新加入版块');
?>
<style type="text/css">
...
<div class="row-fluid sortable ui-sortable">
	<div class="box span12">
		...
		<div class="box-header well" data-original-title="">
			<h2><i class="icon-edit"></i>最新加入版块</h2>
		</div>
		<?php }?>
		<div class="box-content">
			...
			<?php }?>
			<form class="form-horizontal" method="post" action="<?php echo $this->createUrl('index/newdotey',array('op'=>'addNewDotey'));?>" enctype="multipart/form-data">
				<fieldset>
				...
				<dl style="border-top:2px #999999 solid"></dl>
				 <div class="control-group">
				  ...
				  <div class="control-group" style="display:none;" id="show_addEffect">
				      ...
				      <div class="controls">
						  <a href="javascript:;" id="addEffect" class="btn">+添加到侧栏二</a>
				      </div>
				   </div>
...
$(document).ready(function(){
	...
	$("#valid_dotey_info").click(function(){
		...
		if(doteyName){
			...
			$.ajax({
				url:"<?php echo $this->createUrl("index/newdotey");?>",
				type:"post",
				dataType:"text",
	...
	$("#addEffect").click(function(){
		...
		if(doteyId && doteyName && nickName){
			if(effectNum>500){
				alert('最多只能添加500个侧栏推荐二');
			}else{
				if($('#'+doteyId).attr('class') == 'effect' ){
					$("#valid_dotey_info_noty").html(doteyName+' 已经存在于侧栏推荐二中，不能重复添加').show();
