<?php
$this->breadcrumbs = array('主播管理','新增才艺补贴');
?>
<div class="row-fluid sortable ui-sortable">
	<div class="box span12">
		<?php if(!($this->isAjax)){?>
		<div class="box-header well" data-original-title="">
			<h2><i class="icon-edit"></i> 新增才艺补贴</h2>
		</div>
		<?php }?>
		<div class="box-content">
			...
			<?php }?>
			<form class="form-horizontal" action="<?php echo $this->createUrl('dotey/addallowance',array('op'=>'addAllowance'));?>" method="post">
				<fieldset>
				  ...
				  <div class="control-group">
					<label class="control-label" for="focusedInput">补贴理由</label>
					<div class="controls">
...
$(function(){
	...
	$('#submit_award').click(function(){
		if($('#dotey_info_uids').children('.control-group').length <= 0){
			$('#valid_dotey_info_noty').html('补贴对象不能为空').show();
			return false;
		}else{
		...
		if(!$('#form_quantity').attr('value') || isNaN($('#form_quantity').attr('value'))){
			$('#info_form_quantity').html('请填写补贴的金额且值为数字').show();
			return false;
		}else{
		...
		if(!$('#form_reason').attr('value')){
			$('#info_form_reason').html('请填写补贴原因').show();
			return false;
		}else{
	...
	$("#valid_dotey_info").click(function(){
		...
		if(doteyName){
			$("#valid_dotey_info_noty").html("").hide();
			$.ajax({
				url:"<?php echo $this->createUrl("dotey/addallowance");?>",
				type:"post",
				dataType:"text",
