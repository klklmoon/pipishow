$(document).ready(function(){
	...
	{
		var themePath;
		themePath = assertCssPath?assertCssPath+'/':'';
		$("link").each(function(inx){
			if(inx == 0){
				$(this).attr('href',themePath+'/bootstrap-'+theme_name+'.css');
			}
		});
	}
	...
	$('ul.main-menu li').each(function(){
		var tree = $(this).next('li.tree');
		if(tree.attr('class')){
			$(this).addClass('');
			$(this).click(function(){
				$(this).next('li.tree').toggle('normal','swing');
			});
		}
	});
	// 保持ajax效果跳转页面
	$('#uniform-is-ajax span').attr('class','checked');
