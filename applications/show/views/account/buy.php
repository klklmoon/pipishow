...
<?php $this->renderPartial('account_left',array('account_left'=>$account_left,'dotey_left'=>$dotey_leftt,
    ...
    <div class="main fright">
       ...
       <div id="MainCon">
		   ...
		   <?php
			...
			?>
				<ol class="page">
					<li><a href="?r=account/buy">首页</a></li>
			<?php
				$_page = $page > 1 ? ( ($page_num-$page < 2) ? (($page_num - 2 > 0) ? $page_num -2 : 1) : $page - 1) : 1;
				for($_p = $_page; $_p <= $page_num; $_p++){
					echo '<li><a href="?r=account/buy&page='.$_p.'" '.($_p==$page ? 'style="background:#ffb6e2"' : '').'>'.$_p.'</a></li>';
					if(($_p - $_page) == 2) {
						break;
					}
				}
			?>
				<li><a href="?r=account/buy&page=<?php echo $page_num;?>">尾页</a></li>
				</ol>
			<?php
			}else{
				echo '暂无记录';
