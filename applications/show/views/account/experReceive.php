...
<?php $this->renderPartial('account_left',array('account_left'=>$account_left,'dotey_left'=>$dotey_left,'agent_left'=>$agent_left));?>
    ...
    <div class="main fright">
        <ul class="main-menu clearfix" id="MianList">
            ...
            <li class="menuvisted"><a href="#">贡献值</a></li>
           ...
           <div class="cooper-list">
            ...
            <table width="700" border="1" bordercolor="#DDDDDD">
			  ...
			  ?>
				  <tr>
					...
					<td height="42"><?php echo $v['source']=='activity' ? '活动赠送' : '系统赠送';?></td>
					<td height="42">贡献值</td>
					<td height="42"><?php echo $v['dedication'];?></td>
					<td height="42"><?php echo $v['info'];?></td>
				  </tr>
			  <?php
				}
			  ?>
			...
			<?php
				$counts = $count['count'];
					...
					echo '<ol class="page">
							<li><a href="?r=account/propsreceive'.$page_url.'">首页</a></li>';
						$_page = $page > 1 ? ( ($page_num-$page < 2) ? (($page_num - 2 > 0) ? $page_num -2 : 1) : $page - 1) : 1;
						for($_p = $_page; $_p <= $page_num; $_p++){
							echo '<li><a href="?r=account/propsreceive'.$page_url.'&page='.$_p.'" '.($_p==$page ? 'style="background:#ffb6e2"' : '').'>'.$_p.'</a></li>';
							if(($_p - $_page) == 2) {
								break;
							}
						}
					echo	'<li><a href="?r=account/propsreceive'.$page_url.'&page='.$page_num.'">尾页</a></li>
						 </ol>';
			}else{
				echo '没有记录';
			}
