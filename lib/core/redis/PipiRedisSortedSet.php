 ...
 * @author Charles Pick
 * @version $Id: PipiRedisSortedSet.php 8331 2013-03-29 14:44:24Z suqian $ 
 */
class PipiRedisSortedSet extends PipiRedisIterableEntity {
	 ...
	 * @param PipiRedisSortedSet|string $destination the destination to store the result in
	 * @param mixed $set The sets to compare to, either PipiRedisSortedSet instances or their names
	 * @param array $weights the weights for the sets, if any
	 * @return PipiRedisSortedSet a set that contains the intersection between this set and the given sets
	 */
	public function interStore($destination, $set, $weights = null) {
		if ($destination instanceof PipiRedisSortedSet) {
			$destination->_count = null;
			$destination->_data = null;
		}
		else {
			$destination = new PipiRedisSortedSet($destination,$this->getConnection());
		}
		if (is_array($set)) {
			$sets = $set;
		}
		...
		foreach($sets as $n => $set) {
			if ($set instanceof PipiRedisSortedSet) {
				$sets[$n] = $set->name;
			}
		}
	 ...
	 * @param mixed $set The sets to compare to, either PipiRedisSortedSet instances or their names
	 * @param array $weights the weights for the sets, if any
	 * @return PipiRedisSortedSet a set that contains the union of this set and the given sets
	 */
