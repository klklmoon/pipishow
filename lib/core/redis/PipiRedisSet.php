 ...
 * @author Charles Pick
 * @version $Id: PipiRedisSet.php 8331 2013-03-29 14:44:24Z suqian $ 
 */
class PipiRedisSet extends PipiRedisIterableEntity {
	/**
	 ...
	 * @param mixed $set, $set2 The sets to compare to, either PipiRedisSet instances or their names
	...
	public function diff($set) {
		...
		foreach($parameters as $n => $set) {
			if ($set instanceof PipiRedisSet) {
	 ...
	 * @param PipiRedisSet|string $destination the destination to store the result in
	 * @param mixed $set, $set2 The sets to compare to, either PipiRedisSet instances or their names
	 * @return PipiRedisSet a set that contains the difference between this set and the given sets
	 */
	public function diffStore($destination, $set) {
		if ($destination instanceof PipiRedisSet) {
			$destination->_count = null;
			$destination->_data = null;
		}
		else {
			$destination = new PipiRedisSet($destination,$this->getConnection());
		}
		if (is_array($set)) {
			$parameters = $set;
		}
	 ...
	 * @param PipiRedisSet|string $destination the destination to store the result in
	 * @param mixed $set, $set2 The sets to compare to, either PipiRedisSet instances or their names
	 * @return PipiRedisSet a set that contains the intersection between this set and the given sets
	 */
	 ...
	 * @param PipiRedisSet|string $destination the destination to store the result in
	 * @param mixed $set, $set2 The sets to compare to, either PipiRedisSet instances or their names
	 * @return PipiRedisSet a set that contains the union of this set and the given sets
	 */
	 ...
	 * @param PipiRedisSet|string $destination the set to move the item to
