 ...
 * @author Charles Pick
 * @version $Id: PipiRedisRecord.php 8724 2013-04-16 12:07:52Z hexin $ 
 */
abstract class PipiRedisRecord extends CFormModel {

	/**
	 * The redis connection
	 * @var PipiRedisConnection
	 */
	public static $redis;
	/**
	 ...
	 * @return PipiRedisRecord redis record model instance.
	 */
	public static function model($className=__CLASS__)
	{
	 ...
	 * @return PipiRedisConnection the redis connection used by redis record.
	 */
	public function getRedisConnection()
	{
		...
		else
		{
			...
			if(self::$redis instanceof PipiRedisConnection)
				return self::$redis;
			else
				throw new CException(Yii::t('yii','Redis Record requires a "redis" PipiRedisConnection application component.'));
		}
	}
	/**
	 * Sets the redis connection used by this redis record
	 * @param PipiRedisConnection $connection the redis connection to use for this record
	 */
	public function setRedisConnection(PipiRedisConnection $connection) {
	 ...
	 * @return PipiRedisRecord the active record
	 */
	 ...
	 * @return PipiRedisRecord the newly created redis record. The class of the object is the same as the model class.
	...
	public function getRedisKey($pk = null) {
		...
		if(!empty($this->redisKey)) return $this->redisKey.":".$pk;
	 ...
	 * @return PipiRedisRecord the record found. Null if none is found.
	 */
	public function findByPk($pk)
	{
		Yii::trace(get_class($this).'.findByPk()','packages.redis.PipiRedisRecord');
		$this->beforeFind();
		$hash = new PipiRedisHash($this->getRedisKey($pk),$this->getRedisConnection());
		if ($hash->getCount() == 0) {
			return null;
		}
	 ...
	 * @return PipiRedisRecord[] the records found.
	 */
	public function findAllByPk($pks)
	{
		Yii::trace(get_class($this).'.findAllByPk()','packages.redis.PipiRedisRecord');
		$hashes = array();
	 ...
	 * @param PipiRedisIterableHash $redisHash the redis hash
	 */
	 ...
	 * @return PipiRedisIterableHash the redis hash
	 */
	public function getRedisHash()
	{
		if ($this->_redisHash === null) {
			$this->_redisHash = new PipiRedisHash($this->getRedisKey(), $this->getRedisConnection());
		}
	 ...
	 * @param PipiRedisIterableSet $redisSet the redis set
	 */
	 ...
	 * @return PipiRedisIterableSet the redis set
	 */
	public function getRedisSet()
	{
		if ($this->_redisSet === null) {
			$this->_redisSet = new PipiRedisSet(get_class($this), $this->getRedisConnection());
		}
	...
	/**
	 * 
	 * @param PipiRedisCache $redisCache
	 */
	...
	/**
	 * 
	 * @return PipiRedisCache
	 */
	public function getRedisCache(){
		if ($this->_redisCache === null) {
			$this->_redisCache = new PipiRedisCache();
			$this->_redisCache->setConnection($this->getRedisConnection());
		}
