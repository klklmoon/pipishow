 ...
 * @package
 */
class PipiXmlParser  extends CApplicationComponent{
	...
	public function __construct($version = '1.0', $encode = 'utf-8') {
		if (!class_exists('DOMDocument')) throw new CException('DOMDocument is not exist.');
	...
	public function parseToXml($source, $charset = 'utf8') {
		switch (gettype($source)) {
			...
			case 'array':
				...
				break;
			case 'string':
				$source = mb_convert_encoding($source, 'utf8', $charset); 
			default:
	...
	public function getChilds($node) {
		...
		foreach ($node->childNodes as $_node) {
			...
			$nodeName = "" !== ($name = $_node->getAttribute($this->name)) ? $name : $_node->nodeName;
	...
	public function getAttributes($node) {
		...
		foreach ($node->attributes as $attribute) {
			if ($this->name != $attribute->nodeName) {
